...
app.post("/api/assistant", async (req, res) => {
  const { query, location, npk, language } = req.body || {};
  ...
  const prompt = [
    ...
    "Give thorough, detailed practical answers. Always write at least 5 full lines/paragraphs.",
    "Cover: what to do, when to do it, how much/quantity, what to watch for, and any risk or weather consideration.",
    "Use numbered points or bullet points when listing steps or recommendations.",
    ...
    "CRITICAL: Output ONLY the final answer text. Never show your thinking, reasoning, planning, persona notes, constraints, bullet-point metadata, or any internal analysis. Start directly with the answer.",
    language && language !== "en-IN" ? `IMPORTANT: Respond entirely in the language for locale "${language}". Use simple everyday words a farmer understands. Do not mix languages.` : "",
  ...
  try {
    ...
    return res.json({ answer: cleanAssistantAnswer(answer, language) });
  } catch (error) {
...
app.post("/api/disease-detect", async (req, res) => {
  const { partType, image, language } = req.body || {};
  ...
  const textPrompt = [
    ...
    language && language !== "en-IN" ? `IMPORTANT: Write the explanation and recommendation fields in the language for locale "${language}". Use simple everyday words a farmer understands.` : "",
  ].filter(Boolean).join("\n");
...
function cleanAssistantAnswer(answer, language) {
  ...
  // For non-English languages: reasoning is in English/ASCII.
  // Find the first line that contains non-ASCII characters — that's where the real answer starts.
  if (language && language !== "en-IN") {
    const firstNonAsciiIdx = lines.findIndex(l => /[^\x00-\x7F]/.test(l));
    if (firstNonAsciiIdx > 0) {
      text = lines.slice(firstNonAsciiIdx).join("\n").trim();
    }
  }

  // Strip leading greetings
  const answerLines = text.split("\n");
  const cleaned = answerLines.filter((line, idx) => {
...
app.delete("/api/admin/messages/:id", (req, res) => {
  ...
  const idx = communityMessages.findIndex((m) => m.id === id);
  if (idx === -1) return res.status(404).json({ error: "Message not found." });
  communityMessages.splice(idx, 1);
